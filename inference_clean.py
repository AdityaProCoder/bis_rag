"""
BIS Standards RAG Inference Pipeline - Clean Version
Copy of inference_main.py that cleanly mirrors the known-working test_bge.py import order.
Steps [1]-[7]: Pre-retrieval -> Dual retrieval -> Fusion -> Paraphrase -> Rerank -> Validate -> Format
CLI: python inference_clean.py --input hidden_queries.json --output results.json
"""
import os
...
TOP_DENSE   = 20
TOP_BM25    = 20
FUSION_K    = 10   # Top candidates fed into reranker (reranker bypassed if <= RERANK_K)
RERANK_K    = 3
OUTPUT_K    = 3
RRF_K       = 10
GRAPH_BOOST = 0.1
...
def paraphrase_trigger(query, expanded_terms, bm25_top_score):
    # Always fire paraphrase for maximum coverage; cost is ~700ms LLM + 15ms dense encode
...
def format_output(query):
    ...
    if paraphrase:
        try:
            para_dense = retrieve_dense(paraphrase, 50)  # Best: wider paraphrase pool helps cement/sand
    ...
    # Parallelize 3 LLM rationale calls: ~700ms each concurrently vs ~2100ms sequential
...
def main():
    ...
    parser.add_argument("--output", default=str(Path("data/results.json")))
    parser.add_argument("--model", default="google/gemma-4-e2b")
    parser.add_argument("--device", choices=["cpu", "cuda", "auto"], default="cuda")
    parser.add_argument("--disable-rerank", action="store_true")
    parser.add_argument("--rerank-backend", choices=["semantic", "flag"], default="semantic")
